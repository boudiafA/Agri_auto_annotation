...
# def create_tags_prompt(class_name: str, class_description: str) -> str:
#     """Create a prompt specifically for generating tags."""
#     return (
#         f"You are analyzing an agricultural image.\n"
#         f"Image class: {class_name}\n"
#         f"Class description: {class_description}\n\n"
#         f"Generate around 7 descriptive tags for this image.\n"
...
# def create_caption_prompt(class_name: str, class_description: str) -> str:
#     """Create a prompt specifically for generating caption."""
#     return (
#         f"You are analyzing an agricultural image.\n"
#         f"Image class: {class_name}\n"
#         f"Class description: {class_description}\n\n"
#         f"Write a descriptive caption of about 3-5 sentences describing this image.\n"
...
# def validate_tags(response: str) -> tuple:
#     """
#     Validate and extract tags from response.
#     Returns (is_valid, tags_list)
#     """
#     try:
#         response = response.strip()
        
#         # Look for content within square brackets
#         match = re.search(r'\[(.*?)\]', response, re.DOTALL)
#         if match:
#             tags_content = match.group(1)
#         else:
#             tags_content = response
...
#         # Check for reasonable number of tags (3-12)
#         if 3 <= len(unique_tags) <= 12:
#             return True, unique_tags
...
# def validate_caption(response: str) -> tuple:
#     """
#     Validate and extract caption from response.
#     Returns (is_valid, caption_text)
#     """
#     try:
...
#         # Flexible sentence count (2-8 sentences)
#         if 2 <= len(sentences) <= 8:
#             return True, caption
        
#         # If sentence count is off but length is reasonable, still accept
#         if 50 <= len(caption) <= 400:
#             return True, caption
...
#     """Save annotations in structured format (blip2, llava, ram, tag2text)"""
#     base_name = os.path.splitext(image_name)[0]
#     class_path = output_dir / class_name
    
#     # Create subfolders
#     for subfolder in ["blip2", "llava", "ram", "tag2text"]:
...
#     """Process all missing image tasks"""
    
#     if not tasks:
#         print("\n✓ All images already processed!")
#         return
    
#     print(f"\nProcessing {len(tasks)} missing images...")
...
#         for task in tasks:
#             try:
#                 # Create prompts for this specific class
#                 tags_prompt = create_tags_prompt(task.class_name, task.class_description)
#                 caption_prompt = create_caption_prompt(task.class_name, task.class_description)
...
#                     try:
#                         response = get_model_response(task.image_path, tags_prompt, model, processor, args)
#                         valid, tags = validate_tags(response)
#                         if valid:
#                             stats['generated_tags'] += 1
#                             break
...
#                         if attempt == 1:
#                             tags = []
#                             tqdm.write(f"Error generating tags for {task.image_name}: {e}")
#                         time.sleep(0.1)
                
#                 # Generate caption
#                 for attempt in range(2):
#                     try:
#                         response = get_model_response(task.image_path, caption_prompt, model, processor, args)
#                         valid, caption = validate_caption(response)
#                         if valid:
...
#                         if attempt == 1:
#                             caption = ""
#                             tqdm.write(f"Error generating caption for {task.image_name}: {e}")
#                         time.sleep(0.1)
...
#                 pbar.set_postfix({
#                     'tags': stats['generated_tags'],
#                     'captions': stats['generated_captions'],
#                     'failed': stats['failed']
#                 })
#                 pbar.update(1)
...
#     print(f"  Generated tags: {stats['generated_tags']}/{len(tasks)}")
#     print(f"  Generated captions: {stats['generated_captions']}/{len(tasks)}")
...
#     dataset_root = Path(args.image_path)
...
#         description="Optimized Gemma 3 annotation script with parallel scanning"
#     )
...
#                        help="Output directory for structured annotations (blip2/llava/ram/tag2text)")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Optimized Gemma 3 annotation script with combined prompt"
    )
    parser.add_argument("--image-path", type=str, required=True,
                       help="Path to images organized by class folders")
    parser.add_argument("--descriptions-json", type=str, required=True,
