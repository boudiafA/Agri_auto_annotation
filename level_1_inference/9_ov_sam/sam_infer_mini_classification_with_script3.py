...
REVISED: Added proper image dimension tracking and scale factor preservation
UPDATED: Skip folders named "annotations" or "visualizations"
"""

# python sam_infer_mini_classification_with_script3.py --batch_mode --base_input_dir "/mnt/e/Desktop/AgML/datasets_sorted/classification" --base_output_dir "/mnt/e/Desktop/AgML/AgriDataset_GranD_annotation/classification"
...
class ParallelSAMProcessor:
    ...
    def get_image_paths(self, input_path: str) -> List[ImageInfo]:
        ...
        # Skip folders named "annotations" or "visualizations"
        skip_folders = {'annotations', 'visualizations'}
        
        class_folders = [d for d in os.listdir(input_path) 
                        if os.path.isdir(os.path.join(input_path, d)) 
                        and d.lower() not in skip_folders]
        
        if not class_folders:
            raise ValueError(f"No valid class subdirectories found in: {input_path}")
        
        # Log skipped folders if any exist
        all_folders = [d for d in os.listdir(input_path) 
                      if os.path.isdir(os.path.join(input_path, d))]
        skipped_folders = [d for d in all_folders if d.lower() in skip_folders]
        if skipped_folders:
            print(f"Skipping folders: {skipped_folders}")
...
class BatchDatasetProcessor:
    ...
    def get_dataset_folders(self):
        """Get list of dataset folders to process (skip annotations and visualizations)"""
        ...
        dataset_folders = [item for item in os.listdir(self.base_input_dir)
                          ...
                          and item.lower() not in skip_folders]
        
        if not dataset_folders:
            raise ValueError(f"No valid dataset folders found in: {self.base_input_dir}")
        
        # Log skipped folders if any exist
        all_folders = [item for item in os.listdir(self.base_input_dir)
                      if os.path.isdir(os.path.join(self.base_input_dir, item))]
        skipped_folders = [item for item in all_folders if item.lower() in skip_folders]
        if skipped_folders:
            print(f"Skipping dataset folders: {skipped_folders}")
