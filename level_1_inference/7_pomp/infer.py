...
# Handle dataset registration conflicts by monkey-patching both DatasetCatalog and MetadataCatalog
from detectron2.data import DatasetCatalog, MetadataCatalog

# Store the original methods
_original_register = DatasetCatalog.register
_original_metadata_set = MetadataCatalog.get("dummy").set.__class__

def safe_register(name, func):
    """Register function that ignores already registered datasets"""
    if name not in DatasetCatalog.list():
        _original_register(name, func)
    # If already registered, just ignore silently

def safe_metadata_set(self, **kwargs):
    """Metadata set function that ignores conflicts"""
    for k, v in kwargs.items():
        if hasattr(self, k):
            # If attribute already exists, skip setting it to avoid conflicts
            continue
        setattr(self, k, v)

# Temporarily replace both methods
DatasetCatalog.register = safe_register

# Get the Metadata class and patch its set method
from detectron2.data.catalog import Metadata
_original_metadata_set = Metadata.set
Metadata.set = safe_metadata_set

try:
    # Now safely import Detic libraries
    from CenterNet2.centernet.config import add_centernet_config
    from detic.config import add_detic_config
finally:
    # Restore the original methods
    DatasetCatalog.register = _original_register
    Metadata.set = _original_metadata_set
...
def main():
    ...
    for image_name in tqdm(image_names):
        ...
        try:
            tag2text_tags = json.load(open(f"{tags_dir_path}/tag2text/{os.path.splitext(image_name)[0]}.json", 'r'))
        except Exception as e:
            print(f"Tag2Text exception at image {image_name}")
            tag2text_tags = {image_name: {"tag2text": {"tags": []}}}
        try:
            ram_tags = json.load(open(f"{tags_dir_path}/ram/{os.path.splitext(image_name)[0]}.json", 'r'))
        except Exception as e:
