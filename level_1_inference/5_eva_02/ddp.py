...
class CustomImageDataset(Dataset):
    ...
    def __getitem__(self, idx):
        image_path = self.image_paths[idx]
        image_name = os.path.basename(image_path)
        json_file_path = f"{self.output_dir_path}/{self.model_name}/{os.path.splitext(image_name)[0]}.json"
        # Check if the file already exists
        if os.path.exists(json_file_path):
            return image_name, None, None, None

        # inside __getitem__ (replace the block that builds `image`)
        image = self.image_read_func(image_path, format="RGB")
        height, width = image.shape[:2]
        ...
        # CHW float32, contiguous, then torch tensor
        image = np.ascontiguousarray(image.astype("float32").transpose(2, 0, 1))
