...
def annotate(image_source: np.ndarray, boxes: torch.Tensor, logits: torch.Tensor, phrases: List[str]) -> np.ndarray:
    ...
    # Fix: Convert list to numpy array first, then to tensor
...
def run_owl_vit(model, processor, image_path, text_queries, device):
    ...
    # Ensure image is in RGB mode - this fixes the channel dimension issue
    if image.mode != 'RGB':
    ...
    if image_np.ndim == 2:
        ...
        # Single channel but 3D - convert to RGB
        image_np = np.repeat(image_np, 3, axis=2)
        image = Image.fromarray(image_np, mode='RGB')

    # Ensure image is in RGB mode after any conversions
    if image.mode != 'RGB':
        image = image.convert('RGB')
