...
def worker(args, image_names):
    for image_name in tqdm(image_names):
        ...
        for model in MODEL_NAMES:
            json_file_path = f"{args.predictions_dir_path}/{model}/{image_name}.json"
            if not os.path.exists(json_file_path) or is_file_empty(json_file_path):
                continue
            try:
                with open(json_file_path, 'r') as f:
                    json_image_data = json.load(f)
                
                # Get the actual key from the JSON (handles .jpg, .jpeg, .png, etc.)
                actual_key = list(json_image_data.keys())[0]
                
                if not all_data:  # First model - initialize with actual key
                    all_data[actual_key] = {}
                
                if model in ['blip2', 'llava']:
                    if model == 'blip2':
                        json_image_data = json_image_data[actual_key][model]
                    else:
                        llava_data = json_image_data[actual_key][model]
                        if isinstance(llava_data, dict) and model in llava_data:
                            json_image_data = llava_data[model]
                        else:
                            json_image_data = llava_data
                    all_data[actual_key][model] = json_image_data
                elif "mdetr-re" in model:
                    json_image_data = json_image_data[actual_key]["mdetr-re"]
                    if "mdetr-re" in all_data[actual_key].keys():
                        all_data[actual_key]["mdetr-re"] = \
                            all_data[actual_key]["mdetr-re"] | json_image_data
                    else:
                        all_data[actual_key]["mdetr-re"] = json_image_data
