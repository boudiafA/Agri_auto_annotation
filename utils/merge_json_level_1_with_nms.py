...
def worker(args, image_names):
    for image_name in tqdm(image_names):
        ...
        for ext in possible_extensions:
            ...
            # Try to load data for each model
            for model in MODEL_NAMES:
                json_file_path = f"{args.predictions_dir_path}/{model}/{image_name}.json"
                if not os.path.exists(json_file_path):
                    continue
                ...
                try:
                    # Read and parse JSON properly (just once)
                    ...
                    print(f"Exception while processing {json_file_path}: {e}")
                    continue
...
def main():
    ...
    all_image_names = [os.path.splitext(image_name)[0] for image_name in all_image_names]
