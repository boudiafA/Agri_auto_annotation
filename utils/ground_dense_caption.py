...
def process_file(file_names, args):
    for filename in tqdm(file_names):
        output_file_path = f"{args.output_dir_path}/{os.path.splitext(filename)[0]}.json"
        if os.path.exists(output_file_path):
            continue

        level_2_json_file_path = f"{args.level_2_processed_json_path}/{os.path.splitext(filename)[0]}.json"
