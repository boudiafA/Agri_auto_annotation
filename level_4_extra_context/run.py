# (grand_env_9) abood@DESKTOP-EKAJBQH:~/groundingLMM/GranD/level_4_extra_context$
# python run.py --image_dir_path '/home/abood/groundingLMM/GranD/images' --level_2_dir_path "/home/abood/groundingLMM/GranD/level-2-processed_labelled" --output_dir_path "/home/abood/groundingLMM/GranD/level-4-vicuna-13B" --gpu_ids "0" --job_id '111' --model_path "/home/abood/groundingLMM/GranD/checkpoints/vicuna-13b-v1.5" --image_names_txt_path "/home/abood/groundingLMM/GranD/image_names.txt"
import os
import argparse
import subprocess
import time
...
def launch_vicuna(image_names_txt_path, level_2_dir_path, gpu_id, output_dir_path, model_path):
    # Set the environment variable for this specific process
    os.environ["CUDA_VISIBLE_DEVICES"] = str(gpu_id)

    # Launch the external script
    subprocess.run(["python", "query_vicuna_vLLM_level_4.py",
...
def parse_arguments():
    ...
    parser.add_argument("--image_names_txt_path", type=str, required=True,
                        help="Path to text file containing image names")
...
def main():
    ...
    for i, gpu_id in enumerate(gpu_ids):
        ...
        p = Process(target=launch_vicuna,
                    args=(txt_path, args.level_2_dir_path, gpu_id, args.output_dir_path, args.model_path))
        processes.append(p)
        p.start()
