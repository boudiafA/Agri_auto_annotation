...
def get_simple_level_4_scene_graph(image_name, json_contents):
    ...
    try:
        ...
        print(f"JSON structure error: {str(e)}. Trying alternative structure...")
        try:
            # Try direct access if the JSON is not nested under image_name
            objects = json_contents['objects']
            landmark = json_contents['landmark']
        except (KeyError, TypeError):
            print(f"Could not find 'objects' or 'landmark' in JSON for {image_name}")
            objects = []
            landmark = {"category": "unknown", "fine_category": "unknown"}

    # If no objects found, look for "floating_objects"
    if len(objects) == 0:
        try:
            objects = json_contents[image_name].get('floating_objects', [])
        except (KeyError, TypeError, AttributeError):
            try:
                objects = json_contents.get('floating_objects', [])
            except (KeyError, TypeError, AttributeError):
                objects = []
    ...
    # Add landmark information - handle different landmark structures
    scene_graph_string += f"\nLandmarks:\n"
    if isinstance(landmark, dict):
        category = landmark.get('category', 'unknown')
        fine_category = landmark.get('fine_category', 'unknown')
        scene_graph_string += f"{category}, {fine_category}\n"
    elif isinstance(landmark, str):
        scene_graph_string += f"{landmark}\n"
    else:
        scene_graph_string += "unknown\n"
