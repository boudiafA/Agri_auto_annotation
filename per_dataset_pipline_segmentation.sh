...
# Configuration
DATASETS_ROOT="/mnt/e/Desktop/AgML/datasets_sorted/segmentation"
OUTPUTS_ROOT="/mnt/e/Desktop/AgML/AgriDataset_GranD_annotation/segmentation"
...
process_image_directory() {
    ...
    run_in_scoped_env fastvlm level_1_inference/gemma3 \
        ...
        --descriptions_json "../../detailed_class_caption/segmentation/species_discription_cleaned.json" \
...
# Main processing loop - MODIFIED TO ONLY PROCESS "images" SUBFOLDER
for dataset_dir in "$DATASETS_ROOT"/*; do
    ...
    images_subfolder="$dataset_dir/images"
    
    if [ -d "$images_subfolder" ]; then
        if has_images "$images_subfolder"; then
            echo "Found 'images' subfolder with image files in: $dataset_name"
            img_dir="$images_subfolder"
            base_output_dir="$OUTPUTS_ROOT/$dataset_name"
            process_image_directory "$img_dir" "$base_output_dir" "$dataset_name"
        else
            echo "⚠️  Warning: 'images' subfolder exists but contains no image files in: $dataset_name"
        fi
    else
        echo "⚠️  Warning: No 'images' subfolder found in dataset: $dataset_name"
        echo "    Expected path: $images_subfolder"
    fi
