# (grand_env_4) abood@DESKTOP-EKAJBQH:~/groundingLMM/GranD/level_2_inference/5_label_assignment$
# python infer.py --image_dir_path '/home/abood/groundingLMM/GranD/images' --level_2_dir_path "/home/abood/groundingLMM/GranD/output/level-2-processed_gpt4roi" --output_dir_path "/home/abood/groundingLMM/GranD/output/level-2-processed_eva_clip" --local_rank 0
import argparse
import time
...
def run_inference(model, tokenizer, box_crops, labels, is_ddp=False):
    """
    model: Clip model
    ...
    with torch.no_grad(), torch.cuda.amp.autocast():
        # Use model.module only if DDP is enabled
        if is_ddp:
            all_box_features = model.module.encode_image(all_box_tensor.to('cuda'))
            all_label_features = model.module.encode_text(text)
        else:
            all_box_features = model.encode_image(all_box_tensor.to('cuda'))
            all_label_features = model.encode_text(text)
...
def main():
    ...
    # Check if we're using distributed mode
    ...
    if is_distributed:
        sampler = DistributedSampler(image_dataset, rank=args.local_rank, num_replicas=args.world_size, shuffle=False)
    else:
        sampler = None
    ...
    for image_name, box_crops, labels in tqdm(image_dataloader):
        ...
        output_json_file_path = f"{args.output_dir_path}/{os.path.splitext(image_name)[0]}.json"
        if os.path.exists(output_json_file_path):
            continue
        if len(box_crops) == 0:
            continue
        assigned_labels = run_inference(model, tokenizer, box_crops, labels, is_ddp=is_distributed)
