...
pip dependencies fail during conda environment creation:
...
Memory issues during model inference:
...
Environment Isolation: Each environment is fully isolated to prevent dependency conflicts between the different models.

GPU Requirements: Most models require a CUDA-capable GPU for inference. CPU-only inference may be possible for some models but will be extremely slow.

Disk Space: Ensure adequate disk space is available for all environments, model checkpoints (~50GB+), and generated annotations.

Checkpoint Management: It is recommended to keep all downloaded checkpoints in a single, centralized checkpoints directory for easier management.

Multi-GPU Support: Most inference scripts are designed to support multi-GPU processing to accelerate the annotation pipeline.

Next Steps

After successfully setting up all environments:

Download all required model checkpoints into your designated checkpoints directory.

Verify each environment by running a test inference on a single image.

Configure the main run_pipeline.sh script with the correct paths for your system.

Run the complete pipeline or individual components as needed.
