...
# Configuration
DATASETS_ROOT="/mnt/e/Desktop/AgML/datasets_sorted/iNatAg_subset"
OUTPUTS_ROOT="/mnt/e/Desktop/AgML/AgriDataset_GranD_annotation/iNatAg_subset"
CKPT_DIR="/home/abood/groundingLMM/GranD/checkpoints"

# Specific dataset to process
DATASET_NAME="abelmoschus_esculentus"

# Function to handle errors (define based on your needs)
handle_errors() {
    echo "Error in $1"
...
count_files() {
    local dir="$1"
    local pattern="$2"  # Optional: file pattern (e.g., "*.json", "*.txt")
...
should_skip_step() {
    ...
    # Count ALL files in input and output directories (no pattern filtering)
    ...
    # Skip if output has same or more files than input (and output is not empty)
...
# New scoped environment runner
run_in_scoped_env() {
    local env_name="$1"
    local work_dir="$2"
    shift 2 # Remove env_name and work_dir

    # Switch CUDA version in the main shell context so CURRENT_CUDA is updated globally
    switch_cuda_version "$env_name"
    # switch_cuda_version exits on failure, so no explicit check needed here

    ( # Start a subshell to localize conda env and directory changes
        echo "--- Activating conda env: $env_name in subshell ---"
        # CUDA PATHs are already set by switch_cuda_version in parent scope, subshell inherits them.
        source "$(conda info --base)/etc/profile.d/conda.sh"
        if ! conda activate "$env_name"; then
            echo "✗ Failed to activate conda environment: $env_name"
            exit 1 # This exit is for the subshell
        fi
        echo "✅ Conda env $env_name activated."

        # Store original directory from within the subshell's perspective if needed,
        # but cd will use absolute or relative paths from project root effectively.
        # original_subshell_dir=$(pwd) 
        ...
        # echo "--- Returning to directory: $original_subshell_dir (from $(pwd)) ---"
        # cd "$original_subshell_dir" # Not strictly necessary as subshell will exit and restore pwd
        exit $cmd_exit_code # Exit subshell with command's exit code
    ) # End of subshell
    
    subshell_exit_code=$?
    if [ $subshell_exit_code -ne 0 ]; then
         echo "--- Command failed with exit code $subshell_exit_code ---"
         return $subshell_exit_code # Propagate error
    fi
...
# Main processing - single dataset only
echo "=== Processing single dataset: $DATASET_NAME ==="
...
if [ ! -d "$dataset_dir" ]; then
    ...
    echo "  1. DATASETS_ROOT is set correctly: $DATASETS_ROOT"
    echo "  2. The dataset folder 'abelmoschus_esculentus' exists in that directory"
    exit 1
fi

# Check if this directory contains images
if has_images "$dataset_dir"; then
    echo "Found images in dataset folder: $DATASET_NAME"
    img_dir="$dataset_dir"
    base_output_dir="$OUTPUTS_ROOT/$DATASET_NAME"
    process_image_directory "$img_dir" "$base_output_dir" "$DATASET_NAME"
else
    echo "ERROR: No images found in dataset folder: $DATASET_NAME"
    echo "Directory path: $dataset_dir"
    echo "Please ensure the directory contains image files with extensions: jpg, jpeg, png, JPG, JPEG, PNG"
    exit 1
fi

echo ""
echo "=== Processing complete for abelmoschus_esculentus ==="
