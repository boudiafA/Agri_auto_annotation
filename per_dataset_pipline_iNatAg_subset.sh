...
# Configuration
DATASETS_ROOT="/path/to/iNatAg_subset"
OUTPUTS_ROOT="/path/to/iNatAg_subset_annotation"
CKPT_DIR="/path/to/checkpoints"  # Set your checkpoint directory
...
# Function to process a single image directory
process_image_directory() {
    ...
    if ! should_skip_step "SAM" "$IMG_DIR" "$PRED_DIR/sam"; then
        run_in_scoped_env sam level_1_inference/9_ov_sam \
            python sam_infer.py \
            --image_dir_path "$IMG_DIR" \
            --output_dir_path "$PRED_DIR" || handle_errors "sam-$dataset_name"
    fi
    ...
    if ! should_skip_step "Level-3 Dense Captions" "$IMG_DIR" "$PRED_DIR/level-3-vicuna-13B"; then
        run_in_scoped_env grand_env_9 level_3_dense_caption \
...
# Main processing loop - simplified for iNatAg_subset structure
for dataset_dir in "$DATASETS_ROOT"/*; do
    ...
    if has_images "$dataset_dir"; then
        ...
        echo "Warning: No images found in dataset folder: $dataset_name"
    fi
