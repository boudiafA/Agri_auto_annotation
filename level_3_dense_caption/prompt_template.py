...
def get_scene_graph(image_name, json_contents):
    # Same implementation as before, not changing this function
...
def get_simple_scene_graph(image_name, json_contents, proximity_threshold=200):
    # Handle the case where the JSON content has the image name as a key
    if image_name in json_contents:
        actual_content = json_contents[image_name]
    else:
        actual_content = json_contents
    ...
    # Get objects
    if 'objects' in actual_content and actual_content['objects']:
        objects = actual_content['objects']
    elif 'floating_objects' in actual_content and actual_content['floating_objects']:
        objects = actual_content['floating_objects']
    else:
        objects = []
    
    # Sample objects if there are too many
    if len(objects) > 100:
        objects = sample_object(objects)
    
    # Get relationships and captions
    if 'relationships' in actual_content and actual_content['relationships']:
        relations = actual_content['relationships']
    else:
        relations = {'grounding': []}
    
    if 'captions' in actual_content and actual_content['captions']:
        short_captions = actual_content['captions']
    else:
        short_captions = []
    
    # Get landmark safely
    landmark = None
    if 'landmark' in actual_content:
        landmark = actual_content['landmark']

    # Remove objects with no depth information
    objects = [obj for obj in objects if "depth" in obj.keys()]
    
    if not objects:
        # If there are no valid objects, return a simple scene graph
        scene_graph_string = "No valid objects found in the scene.\n"
        ...
        if landmark:
            scene_graph_string += "\nLandmarks:\n"
            # Check if landmark is a dictionary with expected keys
            if isinstance(landmark, dict) and 'category' in landmark and 'fine_category' in landmark:
                scene_graph_string += f"{landmark['category']}, {landmark['fine_category']}\n"
            # If landmark is a string, just add it directly
            elif isinstance(landmark, str):
                scene_graph_string += f"{landmark}\n"
            # Otherwise, convert to string representation
            else:
                scene_graph_string += f"{str(landmark)}\n"
    ...
    if 'grounding' in relations:
        for grounding in relations['grounding']:
            if 'object_ids' in grounding:
                obj_ids = grounding['object_ids']
                if obj_ids:
                    scene_graph_content['relationships'].append({
                        'phrase': grounding['phrase'],
    ...
    if landmark:
        ...
        scene_graph_string += "No landmark information available\n"
